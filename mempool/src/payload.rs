...
struct Runner {
    ...
    size: usize,
    #[allow(dead_code)]   // the dead code warning comes from the hacky fix of https://github.com/asonnino/hotstuff/issues/50
    max_size: usize,
    min_block_delay: u64,
    name: PublicKey,
...
impl Runner {
    ...
    async fn add(&mut self, tx: Transaction) -> Option<Payload> {
        ...
        let ret = Some(self.make().await);   // make sure every tx goes out as a payload immediately!
        ret
    }
