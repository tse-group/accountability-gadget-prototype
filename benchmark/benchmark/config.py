...
class MyCommittee:
    def __init__(self, num_faults, names, consensus_addr, front_addr, mempool_addr, gossip_addr):
        inputs = [names, consensus_addr, front_addr, mempool_addr, gossip_addr]
        assert all(isinstance(x, list) for x in inputs)
        assert all(isinstance(x, str) for y in inputs for x in y)
        assert len({len(x) for x in inputs}) == 1
    ...
    def _build_checkpoint(self):
        node = {}
        for a, n in zip(self.consensus, self.names):
            node[n] = {'name': n, 'stake': 1}
        return {'authorities': node, 'epoch': 1}

    def _build_gossip(self):
        random.seed(2342)
        node = {}
        for i, (a, n) in enumerate(zip(self.gossip, self.names)):
            # print(i, len(self.names) - self.num_faults)
            if i < len(self.names) - self.num_faults:
                # honest
                g = copy.copy(self.names)
                while n in g:
                    g.remove(n)
                random.shuffle(g)
                node[n] = {'name': n, 'address': a, 'connect_to': g[:15]}
            else:
                # adversarial (needs slightly different connectiveness to ensure good connectivity)
                g_hon = self.names[:(len(self.names) - self.num_faults)]
                g_adv = self.names[(len(self.names) - self.num_faults):]
                assert(n not in g_hon)
                assert(n in g_adv)
                while n in g_adv:
                    g_adv.remove(n)
                random.shuffle(g_hon)
                random.shuffle(g_adv)
                # print(g_hon, g_adv, g_hon[:15] + g_adv[:15])
                node[n] = {'name': n, 'address': a, 'connect_to': g_hon[:15] + g_adv[:15]}
            # print(i, node[n])
        return {'nodes': node}
    ...
    #     consensus_authorities = data['consensus']['authorities'].values()
    #     mempool_authorities = data['mempool']['authorities'].values()
    #     gossip_authorities = data['gossip']['nodes'].values()

    #     names = [x['name'] for x in consensus_authorities]
    #     consensus_addr = [x['address'] for x in consensus_authorities]
    #     front_addr = [x['front_address'] for x in mempool_authorities]
    #     mempool_addr = [x['mempool_address'] for x in mempool_authorities]
    #     gossip_addr = [x['address'] for x in gossip_authorities]
    #     return cls(names, consensus_addr, front_addr, mempool_addr, gossip_addr)


class MyLocalCommittee(MyCommittee):
    def __init__(self, num_faults, names, port):
        assert isinstance(names, list) and all(isinstance(x, str) for x in names)
        assert isinstance(port, int)
        size = len(names)
        consensus = [f'127.0.0.1:{port + i}' for i in range(size)]
        front = [f'127.0.0.1:{port + i + size}' for i in range(size)]
        mempool = [f'127.0.0.1:{port + i + 2*size}' for i in range(size)]
        gossip = [f'/ip4/127.0.0.1/tcp/{port + i + 3*size}' for i in range(size)]
        super().__init__(num_faults, names, consensus, front, mempool, gossip)
...
class MyNodeParameters:
    def __init__(self, json):
        ...
        try:
            ...
            inputs += [json['mempool']['max_payload_size']]
            inputs += [json['mempool']['min_block_delay']]
            inputs += [json['checkpoint']['propose_k_deep']]
            inputs += [json['checkpoint']['propose_inter_checkpoint_delay']]
            inputs += [json['checkpoint']['propose_timeout']]
            inputs += [json['checkpoint']['lcmine_slot']]
            inputs += [json['checkpoint']['lcmine_slot_offset']]
            inputs += [json['checkpoint']['lctree_logava_k_deep']]
